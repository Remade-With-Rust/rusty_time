...
| M3 ✅ 2026-08-26 | NTS client + server; SpaceDB persistence (drift, cookies, keys) | **met** — NTS interop PASSES both directions: we authenticate against chronyd and time.cloudflare.com, and chronyd *selects our server as its sync source* (`^*`, 8 cookies, 0 NAKs). Persistence verified across restarts: pre-restart cookies still redeem. See LEDGER.md, incl. three defects the gate found (blake3's C-on-aarch64, a yrs divide-by-zero, an openssl CA:TRUE default) |
| M4 ✅ 2026-08-26 | Server mode + rate limiting + interleaved; `recvmmsg` batching; ops/`rtimec` control plane | **met** — S12a/b/c in the ledger; chronyd syncs from us in **both** plain (offset −4.8 µs) and interleaved (**−66 ns**, `Interleaved: Yes`) modes. ntpd-rs as a second client: pending. Four defects found (port-keyed limiter, churn-defeated limiting, O(n) eviction, two silent interleaved field errors) — plus one in the gate itself, which passed while chrony measured a 362 ms error |
| M5 ✅ 2026-08-26 | macOS + Windows clock drivers, service integration, packaging | **met** — smoke rigs PASS natively on Linux (26 µs) and Windows (11 µs); macOS runs in CI (no Mac here), with its slew arithmetic unit-tested on every host. systemd `Type=notify` + socket activation, WiX MSI + LocalSystem service, launchd plist + universal2 pkg; `release.yml` smoke-tests the artifact it builds. Three defects found (Windows `--control` was inert, daemon/rtimec disagreed on the default control name, socket activation sat in a crate that forbids `unsafe`) |
| M6 ✅ 2026-08-26 | wasm virtual clock + gateway op; npm + status page | **met** — S2-like network (40 ms RTT, 2:1 asymmetry): worst error **6.854 ms** against the 10 ms bound, of which 6.667 ms is unavoidable asymmetry bias, so the estimator adds ~0.19 ms. The real 31 KB wasm module exchanges time with a live gateway (6/6 accepted, forged replies refused). Gateway speaks real NTPv4 over HTTP through the same `build_reply` as UDP. **Pending:** WebTransport (the HTTP fallback of §6.4b shipped instead), disco deploy (Preview), NTS over the gateway |
| M7 ⚠️ 2026-08-26 | HW timestamping, PHC, PPS/SHM/SOCK refclocks; HW1 lab corpus | **partially met — HW1 NOT met.** Verified for real here: PHC (`/dev/ptp0`, −3.6 µs), SHM and SOCK refclocks (offsets exact), kernel `SO_TIMESTAMPING` receive stamps. **PENDING hardware:** PPS (no `/dev/pps*`), NIC hardware timestamping (NIC reports none), and **HW1 itself, which needs the GPS/PPS lab box**. Found a ~110 ns precision defect: storing a reference timestamp and subtracting to recover the offset capped every refclock at ~100 ns |
